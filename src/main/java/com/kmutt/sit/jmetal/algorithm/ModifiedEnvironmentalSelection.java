...
public class ModifiedEnvironmentalSelection<S extends Solution<?>>
	...
	public ModifiedEnvironmentalSelection(Builder<S> builder) {
	...
	public ModifiedEnvironmentalSelection(List<List<S>> fronts, int solutionsToSelect,
	...
	public List<Double> constructHyperplane(List<S> population, List<S> extreme_points) {
		...
		boolean negative_intercept = false;
		...
		if (!duplicate) {
			...
			long c = x.stream().filter(d -> d < 0).count();

			if (c > 0) { // If negative value exists
		...
		if (duplicate || negative_intercept) // cannot construct the unique hyperplane (this is a casual method to deal
												// with the condition)
		{
			for (int f = 0; f < numberOfObjectives; f += 1) {
	...
	public static class Builder<S extends Solution<?>> {
		...
		public ModifiedEnvironmentalSelection<S> build() {
			return new ModifiedEnvironmentalSelection<>(this);
		}
